...
export function GitHubModal({ open, onOpenChange }: GitHubModalProps) {
  ...
  const [limits, setLimits] = useState<GHQueryLimits>(DEFAULT_GH_QUERY_LIMITS)
  const [savingLimits, setSavingLimits] = useState(false)

  // Sync limits from settings
  useEffect(() => {
    if (settings?.gh_query_limits) {
      setLimits({ ...DEFAULT_GH_QUERY_LIMITS, ...settings.gh_query_limits })
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-sidebar sm:max-w-lg max-h-[80vh] overflow-y-auto">
        <DialogHeader>
        ...
        <div className="space-y-4">
              ...
              <div className="space-y-3 pl-1">
                ...
                {missingCount > 0 && (
                  <div className="flex items-center gap-2 text-amber-500 text-sm bg-amber-500/10 p-2 rounded">
                    <AlertTriangle className="w-4 h-4 flex-shrink-0" />
                    <span>
                      {missingCount} webhook{missingCount > 1 ? 's' : ''}{' '}
                      missing. They may have been deleted from GitHub.
                    </span>
                  </div>
                )}
                ...
                {orphanedCount > 0 && (
                  <div className="flex items-center gap-2 text-amber-500 text-sm bg-amber-500/10 p-2 rounded">
                    <AlertTriangle className="w-4 h-4 flex-shrink-0" />
                    <span>
                      {orphanedCount} webhook{orphanedCount > 1 ? 's' : ''} for
                      repos without projects. Consider deleting them.
                    </span>
                  </div>
                )}
          ...
          <div className="space-y-3 border-t pt-3">
            ...
            {limitsExpanded && (
              <div className="space-y-3 pl-1">
                <div className="flex items-start gap-2 text-xs text-muted-foreground bg-muted/30 p-2.5 rounded">
                  <Info className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" />
                  <div>
                    GitHub GraphQL API allows 5,000 points/hour. Each node
                    requested costs ~1 point; nested connections multiply (e.g.
                    10 PRs x 35 checks = 350 nodes). Higher limits give more
                    complete data but consume rate limit faster.
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-2">
                  {(Object.keys(LIMIT_LABELS) as (keyof GHQueryLimits)[]).map(
                    (key) => (
