...
export function TerminalModal({
  ...
  useEffect(() => {
    ...
    if (!terminalId && settings) {
      setForm((prev) => ({
        ...prev,
        shell: settings.default_shell,
      }))
    }
  }, [editTerminal, form.name, settings])
  ...
  const shellChanged =
    isEdit && editTerminal && form.shell !== (editTerminal.shell ?? '')

  const doSave = async (restartShells = false) => {
    setSubmitting(true)
    try {
      ...
      await utils.workspace.terminals.getTerminal.invalidate()
      onClose()
    } catch (err) {
      toastError(err, 'Failed to update project')
    } finally {
      setSubmitting(false)
      setConfirmShellChange(false)
    }
  }
  ...
  return (
    <>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {!hasGitRepo && !isEdit && (
              <div className="space-y-1">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Git branch and repo will be detected in this path for PR
                  status
                </p>
              </div>
            )}
      ...
      <ConfirmModal
        open={confirmShellChange}
        title="Shell Changed"
        message="Kill active shells to apply changes?"
        confirmLabel="Restart Shells"
        variant="danger"
        onConfirm={() => doSave(true)}
        onCancel={() => setConfirmShellChange(false)}
        secondaryAction={{
          label: 'Save Only',
